      ...
      const fetchData = async () => {
        try {
          const res = await fetch(
            `https://gadget-grid-server.vercel.app/cart?email=${user?.email}`
          );
      ...
      {loading ? (
        ...
        <section className=" mb-32">
          <div className="bg-gradient-to-l h-40 flex justify-center items-center backdrop-blur-lg  from-primary/30 to-secondary/30 w-full">
            <h2 className="text-center py-6  text-web-dark font-extrabold text-4xl uppercase">
              Products In your cart
            </h2>
          </div>
          {loaderData.length === 0 ? (
            <div className="h-screen flex justify-center items-center w-full text-center">
              <p className="text-2xl font-bold text-web-dark">
                ...
                <table className="table w-full">
                  ...
                  <thead className="border-b-2 border-gray-700">
                    <tr className="text-lg text-web-dark">
                      ...
                      <th
                        onClick={() => handleAllDelete(userUid)}
                        className="flex gap-1 cursor-pointer items-center text-base font-semibold bg-red-400 w-max text-white rounded shadow active:scale-95"
                      >
                        <span>
                          <svg
                  ...
                  <tbody>
                    ...
                    {loaderData?.map((cart, idx) => (
                      <tr key={cart._id}>
                        ...
                        <td>
                          <img
                            className="h-16 w-[66px] drop-shadow"
                            src={cart.image}
                            alt={`image of ${cart.products[0]}`}
                          />
                        </td>
                        <td className="first-letter:uppercase text-web-dark ">
                          {cart.products[0]}
                        </td>
                        <td className="font-medium text-web-dark">
                          $ {cart.products[1]}
                        </td>
                        <td>
                          <div
                            onClick={() => handleDelete(cart._id)}
                            className="inline-block cursor-pointer active:scale-95 "
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              fill="none"
